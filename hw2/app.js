...
do {
  ...
  if (Number.isNaN(userInput)) {               //ввели не число --> спробуйте ще раз
    ...
    } else if (userInput === 0) {              //пропускаємо перший(пустий) елемент
  } else {
...
if (numbers.length !== 0) {
  //довжина масиву чисел більша 0 --> Виводимо суму
...
do {
  ...
  for (const value of passwords) {
    //порівнюємо введені дані з множиною паролів
    if (pas === null) {
      //нажато Cancel --> вихід з циклу
      break;
    }
    if (pas === value) {
      //введенне співпало з одним із паролів
  ...
  if (attempts !== 0 && !confirm && pas !== null) {
    //якщо доступ не дозволенно і не нажато Cancel і залишились спроби --> Спробуйте ще раз
    alert(`Неверный пароль, у вас осталось ${attempts} попыток`);
  }
} while (attempts !== 0 && !confirm && pas !== null); //Повторюємо пики: є спроби, доступ не дозволенно, не нажато Cancel

if (attempts === 0 && !confirm && pas !== null) {
  //спроби закінчились, доступ не дозволенно, не нажато Cancel --> Спроби закінчились
