...
const isLoginValid = function(login) {   //перевірка чи aргумент не більший 16 і не менший 4
  const loginArr = login.split("");
  let lenghtLogin = 0;
  for (const value of loginArr) {
    lenghtLogin++;
  }
  if (lenghtLogin >= 4 && lenghtLogin <= 16) {
    return true;                         //менший 4-х або більший 16-ти --> повертає true
  } else {
    return false;                        //інакше --> повертає false
  }
};
...
const isLoginUnique = function(allLogins, login) { // приймає масив наявних логінів і логін, перевіряє чи він не входить в цей масив 
  ...
  for (const value of allLogins) {
    if (value === login) {                         //логін співпадає з значенням в масиві?
      unique = false;                              //--> так--> визначаємо що це не унікальний логін 
      break;
    }
  }
  return unique;                                   //повертаємо значення, чи унікальний логін
};

const addLogin = function(allLogins, login) {      //приймаємо масив наявних логінів і новий логін
  if (!isLoginValid(login)) {                      //первірка на коректну кількість симолів в login, false--> ошибка 
    return "Ошибка! Логин должен быть от 4 до 16 символов";
  } else if (isLoginUnique(allLogins, login)) {    //первірка чи не входить login в масив наявних логінів, false-->уже используется 
    allLogins.push(login);                         //true--> успешно добавлен
